...
async def game_loop(
    ...
    if start_new_game:
        ...
        tiles_1 = user_tiles[consts.user_ids[player_A]]
        tiles_2 = user_tiles[consts.user_ids[player_B]]
        player_1.set_tiles(tiles_1)
        player_2.set_tiles(tiles_2)
    move_result = {}
    if whos_turn == consts.player_nums[player_A]:
        move_result = await play(player_1, word_finder, game_scraper, iteration)
        whos_turn = consts.player_nums[player_B]
    elif whos_turn == consts.player_nums[player_B]:
        move_result = await play(player_2, word_finder, game_scraper, iteration)
        whos_turn = consts.player_nums[player_A]
    else:
        logger.log("No move played detected")
    did_pass = move_result['did_pass']
    is_over = move_result['is_over']
    if did_pass and did_last_player_pass:
        is_over = True
...
async def play(player, word_finder, game_scraper, iteration):
    if iteration >= 2:
        retries = 0
        is_game_loaded = player.check_if_game_loaded()
        while is_game_loaded is False:
            await asyncio.sleep(3)
            is_game_loaded = player.check_if_game_loaded()
            retries += 1
            if not is_game_loaded and retries > 3:
                retries = 0
    ...
    move, letters_played = player.start_play_move(moves, board, tile_images)
    await asyncio.sleep(1)
    player.restart_app()
    if move is False:
        return {'did_play': False, 'did_pass': True, 'is_over': False}
    await word_finder.add_move(move)
...
async def initialize_new_game(whos_turn=consts.player_nums[player_A]):
    player_1 = Player(consts.player_nums[player_A])
    player_2 = Player(consts.player_nums[player_B])
    player_to_start_new_game = player_1 if whos_turn == consts.player_nums[player_A] else player_2
    player_to_accept_game = player_2 if whos_turn == consts.player_nums[player_A] else player_1
    player_to_start_new_game.start_new_game()
    await asyncio.sleep(8)
    player_to_start_new_game.press_pass_move()
    player_to_start_new_game.restart_app()
...
async def start_game(game_scraper, word_finder):
    _, board, user_tiles = await game_scraper.get_game_data()
    player_1_tiles = user_tiles[consts.user_ids[player_A]]
    player_2_tiles = user_tiles[consts.user_ids[player_B]]
    player_1 = Player(
        consts.player_nums[player_A],
        player_1_tiles,
    )
    player_2 = Player(
        consts.player_nums[player_B],
        player_2_tiles
    )
...
async def main():
    try:
        ...
        game_scraper = GameScraper(playwright_manager, player_A, player_B)
